...
tag_t ves_queryCost(char *partID)
{
...
tag_t ves_createCostRev(const char *costID, const char *costName)
{
	TC_write_syslog("\n[VF] ENTER %s", __FUNCTION__);
...
void ves_copyCostFormsToCostObject(const std::vector<tag_t> &costForms, const tag_t &partRev, char *itemID, vector<tag_t> &costRevs)
{
	TC_write_syslog("\n[VF] ENTER %s", __FUNCTION__);
	
	int iRetCode = ITK_ok;
	// search cost object
	tag_t costRev = ves_queryCost(itemID);	
	...
	if (costRev == NULL)
	{
		...
		costRev = ves_createCostRev(itemID, partRevName);
...
void ves_fixCostFormsName(const vector<tag_t> &costRevs)
{
	TC_write_syslog("\n[VF] ENTER %s", __FUNCTION__);
...
void ves_createCostForms(tag_t ecrRev, tag_t tFormRel, std::vector<std::string> relations, std::map<tag_t, int> &itemsCreatedCostForms, bool checkIfUnique)
{
	TC_write_syslog("\n[VF] ENTER %s", __FUNCTION__);
		...
		for(int iNum = 0; iNum < iProp; iNum++)
		{
			...
			if(isCreateForms && processedParts.find(ptProp[iNum]) == processedParts.end() && (!checkIfUnique || itemsCreatedCostForms.find(tItem) == itemsCreatedCostForms.end()))
			{
				...
				if(iRetCode == ITK_ok && tItem != NULLTAG)
				{
					...
					CHECK_ITK(iRetCode,TCTYPE_find_type ("VF4_manufacturing","Form",&tManuForm));
					...
					CHECK_ITK(iRetCode,TCTYPE_find_type ("VF4_logistics","Form",&tLogForm));
					...
					CHECK_ITK(iRetCode,TCTYPE_find_type ("VF4_engineering","Form",&tEngForm));
					...
					CHECK_ITK(iRetCode,TCTYPE_find_type ("VF4_purchasing","Form",&tPurForm));
			...
			if (tManuCostForm && tPurCostForm && tLogCostForm && tEngCostForm && pcItem) 
			{
				...
				ves_copyCostFormsToCostObject(costForms, ptProp[iNum], pcItem, costRevs);
			}
...
extern int VES_AH_attach_cost_form(EPM_action_message_t msg)
{
	...
	char *costFormRelation          = "VF4_change_forms";
	char *changeType               = "VF4_VinES_ECRRevision";
    ...
    if(tRootTask != 0)
    {
        ...
        for(int iInd = 0; iInd < iTgt; iInd++)
		{
            ...
            if(tc_strcmp(pcObjType,changeType) == 0 )
            {
                ...
                {
					//TC_write_syslog("\n 222222222222");
					ves_createCostForms(ptTgt[iInd], tFormRel, relations, itemsCreatedCostForms, false);
					ves_createCostForms(ptTgt[iInd], tFormRel, secondRelations, itemsCreatedCostForms, true);
                }
                else
                {
