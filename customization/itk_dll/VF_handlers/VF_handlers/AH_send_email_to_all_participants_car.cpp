	...
	if (tRootTask != NULLTAG)
	{
		...
		for (int iInd = 0; iInd < iTgt; iInd++)
		{
			...
			if (lItemRevision == true)
			{
				...
				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf6_vehicle_group", &vehicle_group));
				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf6_module_group_comp", &module_group));
				Teamcenter::scoped_smptr<char*> implementationPhases;
				int implementationPhasesNum = 0;
				CHECK_ITK(iRetCode, AOM_ask_value_strings(ptTgt[iInd], "vf6_implementation_date", &implementationPhasesNum, &implementationPhases));
				if (implementationPhasesNum > 0) {
					...
					CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf6_effective_proposal", &effective_old));
