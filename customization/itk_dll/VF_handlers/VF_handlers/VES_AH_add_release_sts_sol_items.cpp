...
bool ves_checkStampByNewPartStatus(tag_t rev, char* isDebug)
{
...
extern int VES_AH_add_release_sts_sol_items(EPM_action_message_t msg)
{
	...
	if (tRootTask != 0)
	{
		...
		for (int iInd = 0; iInd < iTgt; iInd++)
		{
			...
			if (tc_strcmp(pcTgtType, "VF4_VinES_ECNRevision") == 0)
			{
				// new part status
				if (ecnRev == NULL)
				{
					ecnRev = ptTgt[iInd];
					tag_t lovTag = NULL;
					CHECK_ITK(iRetCode, AOM_ask_value_string(ecnRev, "vf4_new_parts_status", &newPartsStatus));
				}
				else
				{
					// 2 MCNs in target
			...
			if (tc_strcmp(pcObjType, pcTgtType) == NULL)
			{
				...
				if (tRelation != 0)
				{
					...
					for (int iCnt = 0; iCnt < iSolItems; iCnt++)
					{
						...
						bool isFirstRevision = ves_checkStampByNewPartStatus(ptSol[iCnt], isDebug);
						if (!isFirstRevision)
						{
