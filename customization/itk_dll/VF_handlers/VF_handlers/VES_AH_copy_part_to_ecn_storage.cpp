...
extern int VES_AH_copy_part_to_ecn_storage(EPM_action_message_t msg)
{
	...
	if (tRootTask != 0)
	{
		...
		for (int i = 0; i < iTgt; i++)
		{
			...
			if (tc_strcmp(ecrRevType.getString(), ecnObjType) == 0 || tc_strcmp(ecrRevType.getString(), "VF4_VinES_ECRRevision") == 0)
			{
				...
				else if (tc_strcmp(ecrRevType.getString(), "VF4_VinES_ECRRevision") == 0)
				{
				...
				if (iCountSolutionObjs > 0)
				{
					...
					if (iRetCode == ITK_ok)
					{
						...
						for (int j = 0; j < iCountSolutionObjs; j++)
						{
							...
							CHECK_ITK(iRetCode, AOM_append_table_rows(ptTgt[i], "vf4_dde_store_table", 1, &index, &tableItems));
							CHECK_ITK(iRetCode, AOM_refresh(tableItems[0], TRUE));
							CHECK_ITK(iRetCode, AOM_set_value_tag(tableItems[0], "vf4_object_reference", ptSolutionObjs[j]));
							CHECK_ITK(iRetCode, AOM_set_value_string(tableItems[0], "vf4_is_send", isSend.c_str()));
							// get item
							tag_t item = NULLTAG;
							...
							CHECK_ITK(iRetCode, AOM_set_value_string(tableItems[0], "vf4_vendor_code", supplierCode.getString()));
							CHECK_ITK(iRetCode, AOM_set_value_string(tableItems[0], "vf4_vendor_name", supplierName2));
