	...
	if (tRootTask != 0)
	{
		...
		for (int iInd = 0; iInd < iTgt; iInd++)
		{
			...
			if (lItemRevision == true)
			{
				...
				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf4_change_reason", &changeReason));
				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf4_es_classification", &ecnClassification));
				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf4_es_priority", &ecnPriority));
				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf4_model_es", &model));

				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf4_es_exchange_newpart", &exchangeabilityNewPart));
				...
				CHECK_ITK(iRetCode, AOM_ask_value_string(ptTgt[iInd], "vf4_es_exchange_oldpart", &exchangeabilityOldPart));
...
/*ves_split string by sign ';'*/
string ves_split(string str, string delimiter) {
	size_t pos = 0;
	string token;
