...
public class VESBPDesignPartCreate_Handler extends AbstractHandler {
	private TCSession session;
	private VESBPDesignPartCreate_Dialog dlg;
	private TCComponent selectedObject = null;
	private static String OBJECT_TYPE = "VF4_BP_Design";
	...
	private LinkedHashMap<String, String> batteryPackCarDataForm;
	private LinkedHashMap<String, String> batteryPackEScooterDataForm;
	private LinkedHashMap<String, String> batteryPackBATDataForm;
	...
	public VESBPDesignPartCreate_Handler() {
		super();
	}
	...
	public Object execute(ExecutionEvent arg0) throws ExecutionException {
		try {
			...
			isFirstCheck = true;
			if (command.contains("newBPPartFromBasePart")) {
				isFromBasePart = true;
				...
				if (targetComp[0] instanceof TCComponentBOMLine) {
					...
					if (objectType.compareToIgnoreCase("VinES BP Design Revision") != 0) {
			...
			partNumberRuleMapping.put("C Car", new HashMap<String, String>() {
				{
					put("Design Part New", "");
					put("Design Part After PR", "");
					put("Sample Part New", "");
					put("Sample Part After PR", "");
					put("Software Part New", "A");
					put("Software Part After PR", "A");
				}
			});

			partNumberRuleMapping.put("D Car", new HashMap<String, String>() {
				{
					put("Design Part New", "");
					put("Design Part After PR", "");
					put("Sample Part New", "");
			...
			partNumberRuleMapping.put("LFP1", new HashMap<String, String>() {
				{
					put("Design Part New", "");
					put("Design Part After PR", "");
					put("Sample Part New", "");
			...
			String[] preValues = TCExtension.GetPreferenceValues("VES_BP_DESIGN_PART_ID_GENERATOR", session);
			if (preValues != null) {
			...
			String[] typeDataForm = new String[] { "Design Part", "Sample Part", "Software Part" };
			LinkedHashMap<String, String> donorVehicleDataForm = TCExtension.GetLovValueAndDisplay("vf4_donor_vehicle", OBJECT_TYPE, session);
			...
			String[] batteryPackDataForm = TCExtension.GetLovValues("VF4_BP_Type");
			String[] groupNameDataForm = TCExtension.GetLovValues("VF4_BP_Group_Name");
			batteryPackCarDataForm = TCExtension.GetLovValueAndDisplay("VF4_BP_Model_Car");
			batteryPackEScooterDataForm = TCExtension.GetLovValueAndDisplay("VF4_BP_Model_EScooter");
			batteryPackBATDataForm = TCExtension.GetLovValueAndDisplay("VF4_BP_Model_Ventilator");
			String[] moduleGrpLov = TCExtension.GetLovValues("VL5_module_group", "BOMLine", session);
			// Init UI
			dlg = new VESBPDesignPartCreate_Dialog(new Shell());
			dlg.create();
			dlg.setTitle("Create VINES BP Design Part");
			...
			dlg.cbBatteryPack.setItems(batteryPackDataForm);
			dlg.cbBatteryPack.addModifyListener(new ModifyListener() {
				@Override
				public void modifyText(ModifyEvent arg0) {
					if (!dlg.cbBatteryPack.getText().isEmpty()) {
						dlg.cbModel.removeAll();

						String batteryPack = dlg.cbBatteryPack.getText();
						try {
							if (batteryPack.compareTo("Car") == 0) {
								StringExtension.UpdateValueTextCombobox(dlg.cbModel, batteryPackCarDataForm);
							} else if (batteryPack.compareTo("EScooter") == 0) {
								StringExtension.UpdateValueTextCombobox(dlg.cbModel, batteryPackEScooterDataForm);
							} else if (batteryPack.compareTo("VES BAT") == 0) {
								StringExtension.UpdateValueTextCombobox(dlg.cbModel, batteryPackBATDataForm);
							}
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	private void updatePartIDFormat() {
		...
		if (type.isEmpty() || model == null || model.isEmpty())
			return;
		...
		if (dlg.txtID.getText().isEmpty()) {
			...
			dlg.txtID.setText(generateNextID(groupName + partNumberGeneratorMapping.get(model)));
		}
	}
	...
	private Boolean checkRequired() {
		...
		if (dlg.txtID.getText().isEmpty()) {
			if (dlg.cbBatteryPack.getText().isEmpty()) {
				warningLabel(dlg.lblBatteryPack, true);
				check = false;
			} else {
				warningLabel(dlg.lblBatteryPack, false);
			}
	...
	private String generateNextID(String prefix) {
		try {
			...
			int id = 0;
			String split = queryOutput[0].toString().substring(8, 11);

			if (id < Integer.parseInt(split))
				id = Integer.parseInt(split);

			return prefix + StringExtension.ConvertNumberToString(id + 1, 3);
		} catch (Exception e) {
			e.printStackTrace();
		}
