...
public class SearchRawMaterial_Dialog extends Dialog {
	private TCSession session = null;
	public Table tblSearch;
	private Button ok;

	private Text txtPartNumber;
	private Text txtName;

	public LinkedList<TCComponent> itemSearch = null;

	public SearchRawMaterial_Dialog(Shell parent) {
		super(parent);
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblPartNumber.setText("ID:");

		txtPartNumber = new Text(container, SWT.BORDER);
		txtPartNumber.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));
		...
		lblMaterial.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));
		
		Combo cbMaterial = new Combo(container, SWT.NONE);
		cbMaterial.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		new Label(container, SWT.NONE);
		
		Label lblCoating = new Label(container, SWT.NONE);
		lblCoating.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		lblCoating.setText("Coating:");
		lblCoating.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));
		
		Combo cbCoating = new Combo(container, SWT.NONE);
		cbCoating.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		new Label(container, SWT.NONE);
		...
		TableColumn partDetails = new TableColumn(tblSearch, SWT.CENTER);
		partDetails.setResizable(true);
		partDetails.setWidth(110);
		partDetails.setText("Part Number");

		TableColumn partType = new TableColumn(tblSearch, SWT.LEFT);
		partType.setResizable(true);
		partType.setWidth(170);
		partType.setText("Part Name");
		
		TableColumn tblclmnMaterial = new TableColumn(tblSearch, SWT.CENTER);
		tblclmnMaterial.setWidth(120);
		tblclmnMaterial.setText("Material");
		tblclmnMaterial.setResizable(true);
		
		TableColumn tblclmnCoating = new TableColumn(tblSearch, SWT.CENTER);
		tblclmnCoating.setWidth(120);
		tblclmnCoating.setText("Coating");
		tblclmnCoating.setResizable(true);

		txtPartNumber.addListener(SWT.KeyUp, new Listener() {
			public void handleEvent(Event evt) {
				tblSearch.deselectAll();
				if (evt.keyCode == SWT.CR || evt.keyCode == SWT.KEYPAD_CR) {
					loadObjects();
	...
	public void loadObjects() {
		...
		String revision = txtName.getText();
