...
public class SearchVendor_Dialog extends Dialog {
	...
	public SearchVendor_Dialog(Shell parent) {
		super(parent);
	...
	protected Control createDialogArea(Composite parent) {
		...
		Label lblVendorCode = new Label(container, SWT.NONE);
		lblVendorCode.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblVendorCode.setText("Vendor Code:");
		lblVendorCode.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));

		txtCode = new Text(container, SWT.BORDER);
		txtCode.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));
	...
	public void loadObjects() {
		...
		String vendorCode = txtCode.getText();
		...
		if (name.isEmpty() && email.isEmpty()) {
			...
			objects = Query.queryItem(session, queryInput, "Vendor");
			...
			if (objects != null) {
				...
				for (TCComponent obj : objects) {
					try {
						itemSearch.add(obj);
						String vendorName = obj.getPropertyDisplayableValue("current_name");
						String vendorEmail = obj.getPropertyDisplayableValue("email_addr");
						setTableRow(new String[] { vendorName, vendorEmail });
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
