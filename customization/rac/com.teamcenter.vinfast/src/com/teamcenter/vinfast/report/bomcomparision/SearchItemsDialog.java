...
public class SearchItemsDialog extends AbstractSWTDialog {

	private Table tblSearch = null;
	private Button ok = null;
	private TableItem[] searchItems = null;
	private LinkedHashMap<String, TCComponent> mapItemsList = null;

	public SearchItemsDialog(Shell parent, int style) {
		super(parent, style); 
		// TODO Auto-generated constructor stub
		setBlockOnOpen(false);
		mapItemsList = new LinkedHashMap<String, TCComponent>();
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		Label lblSORRevID = new Label(container, SWT.NONE);
		GridData gd_lblSORRevID = new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1);
		gd_lblSORRevID.widthHint = 100;
		lblSORRevID.setLayoutData(gd_lblSORRevID);
		lblSORRevID.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));
		lblSORRevID.setText("Revision ID");

		Text txtSORRevID = new Text(container, SWT.BORDER);
		txtSORRevID.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));
		GridData gd_txtSORRevID = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
		gd_txtSORRevID.widthHint = 50;
		txtSORRevID.setLayoutData(gd_txtSORRevID);
		new Label(container, SWT.NONE);
		new Label(container, SWT.NONE);
		...
		txtSORNumber.addListener(SWT.KeyUp, new Listener() {
			...
			public void handleEvent(Event evt) {
				if (evt.keyCode == SWT.CR || evt.keyCode == SWT.KEYPAD_CR) {
					queryDesignRevision(session, txtSORNumber, txtSORRevID, txtSORName, cbOwner);
				}
			}
		});
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText("Items Search Dialog");
	}
	...
	public LinkedHashMap<String, TCComponent> getMapDesignRevCompo() {
		return mapItemsList;
	}

	private void queryDesignRevision(TCSession session, Text itemID, Text itemRevID,Text itemName, Combo cbOwner) {
		TCComponent[] objects = null;
		String id = itemID.getText();
		String revId = itemRevID.getText();
		...
		if (id.equals("") && name.equals("") && owningUser.equals("")) {
			...
			queryInput.put("Revision", revId);
			queryInput.put("Name", name);
			queryInput.put("Owning User", owningUser);
			queryInput.put("Type", PropertyDefines.TYPE_SVC_CHAPTER_REVISION);
			...
			if (objects != null) {
				...
				for (TCComponent obj : objects) {
					try {
						...
						mapItemsList.put(propValues[0], obj);
