...
public class SearchDesignRevDialog extends AbstractSWTDialog {

	private Table tblSearch = null;
	private Button ok = null;
	private TableItem[] searchItems = null;
	private LinkedHashMap<String, TCComponent> mapDesignRevCompo = null;

	public SearchDesignRevDialog(Shell parent, int style) {
		super(parent, style);
		// TODO Auto-generated constructor stub
		setBlockOnOpen(false);
		mapDesignRevCompo = new LinkedHashMap<String, TCComponent>();
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		btnSearch.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event e) {
				if ( !txtSORNumber.getText().isEmpty()){
					queryDesignRevision(session, txtSORNumber, txtSORName, cbOwner);
				}else
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText("Design Search Dialog");
	}
	...
	private void queryDesignRevision(TCSession session, Text itemID, Text itemName, Combo cbOwner) {
		...
		if (id.equals("") && name.equals("") && owningUser.equals("")) {
			...
			queryInput.put("Type", "VF4_DesignRevision;VF4_scooter_vehRevision;VF4_scooter_modRevision;VF3_Scooter_partRevision");
			...
			if (objects != null) {
				...
				for (TCComponent obj : objects) {
					try {
						String[] propValues = obj
								.getProperties(new String[] { "object_string", "owning_user", "object_type" });
						mapDesignRevCompo.put(propValues[0], obj);
