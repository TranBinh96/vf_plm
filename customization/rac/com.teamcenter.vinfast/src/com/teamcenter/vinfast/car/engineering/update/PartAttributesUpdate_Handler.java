...
public class PartAttributesUpdate_Handler extends AbstractHandler {
	private TCSession session;
	private TCComponentItem selectedObject = null;
	private PartAttributesUpdate_Dialog dlg;
	...
	private String[] OBJECT_TYPE_AVAILABLE = new String[] { "VF4_Design", "VF4_JF_Part", "VF4_FlexDesign" };

	public PartAttributesUpdate_Handler() {
		super();
	}
	...
	public Object execute(ExecutionEvent arg0) throws ExecutionException {
		...
		try {
			...
			LinkedHashMap<String, String> coloredPartDataForm = TCExtension.GetLovValueAndDisplay("vf4_colored_part", "VF4_Design", session);
			// ---------------------------- UI -------------------------------------
			dlg = new PartAttributesUpdate_Dialog(new Shell());
			dlg.create();
			...
			StringExtension.UpdateValueTextCombobox(dlg.cbColoredPart, coloredPartDataForm);
			...
			dlg.btnInfoColorCode.setVisible(false);
			dlg.btnInfoColoredPart.setVisible(false);
	...
	private void updateItem() {
		try {
			...
			String coloredPart = (String) dlg.cbColoredPart.getData(dlg.cbColoredPart.getText());
			...
			if (dlg.txtColorCode.getEnabled())
				input.put("vl5_color_code", colorCode);
			if (dlg.cbColoredPart.getEnabled())
				input.put("vf4_colored_part", coloredPart);
...
//			boolean isAFSCheck = false;
			...
			if (relRevs.length == allRevs.length) {
				...
				dlg.txtColorCode.setEnabled(false);
				dlg.cbColoredPart.setEnabled(false);
				...
				dlg.btnInfoColorCode.setVisible(true);
				dlg.btnInfoColorCode.setToolTipText(mess);
				dlg.btnInfoColoredPart.setVisible(true);
				dlg.btnInfoColoredPart.setToolTipText(mess);
...
//			}
	...
	private void updateCurrentValue() {
		try {
			...
			String colorCode = selectedObject.getPropertyDisplayableValue("vl5_color_code");
			String coloredPart = selectedObject.getPropertyDisplayableValue("vf4_colored_part");
			...
			dlg.txtColorCode.setText(colorCode);
			dlg.cbColoredPart.setText(coloredPart);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
