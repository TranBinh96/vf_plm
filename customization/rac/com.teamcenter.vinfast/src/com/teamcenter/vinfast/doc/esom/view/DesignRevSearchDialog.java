...
public class DesignRevSearchDialog extends AbstractSWTDialog{
	...
	protected Control createDialogArea(Composite parent) {
		...
		Text textRevision = new Text(container, SWT.BORDER);
		textRevision.setFont(SWTResourceManager.getFont("Calibri", 11, SWT.NORMAL));
		GridData gd_text_1 = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
		gd_text_1.widthHint = 20;
		textRevision.setLayoutData(gd_text_1);
		
		tablePartNumber = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.SINGLE);
		...
		textID.addListener(SWT.KeyUp, new Listener() {
			...
			public void handleEvent(Event evt) {
				...
				if(evt.keyCode == SWT.CR || evt.keyCode == SWT.KEYPAD_CR) {
					...
					String Rev = textRevision.getText();
					loadObjects(session,ID,Rev);
				}
				 ok.setEnabled(false);
			}
		});
		
		textRevision.addListener(SWT.KeyUp, new Listener() {
	...
	public void loadObjects(TCSession session, String ID, String rev) {
		TCComponent[] objects = null;

		if(ID.equals("") && rev.equals("")) {
			...
			String[] entries = new String[] {"Item ID","Revision", "Type"};
			String[] values = new String[] {ID,rev,"*Design*"};
			objects = new Utilities().searchObjects(session, entries, values, "Item Revision...");
