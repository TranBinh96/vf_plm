...
public class ESOMBulkUpdateECRHandler extends AbstractHandler {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					if (masterData.size() > 0) {
						try {
							...
							propDescInfo.typeName = "VF3_ESOM_ECR";
							...
							for(int i = 0; i < currEcrTable.size(); i++) {
								...
								if(masterData.containsKey(ecrNum)) {
									LinkedHashMap<String, String> prop2Value = masterData.get(ecrNum);
									...
									for (Entry<String, String> entry : prop2Value.entrySet()) {
										...
										TCComponentType objType = ecrDetailInfo.getTypeComponent();
...
//											}
										...
										ServiceData serviceData = DataManagementService.getService(session).setProperties(new TCComponent[] { ecrDetailInfo }, propertyMap);
	...
	public int readCsv(String csvFile) {
		...
		try {
			...
			while ((tempArr = csvReader.readNext()) != null) {
				...
				if (counter == 1) {
					if (tempArr[0].compareToIgnoreCase("ECR Number") != 0) {
						return counter;
					} else {
						for(String displayName : tempArr) {
							...
							}else if(displayName.trim().compareToIgnoreCase("ECR Number") == 0) {
								propRealName.add("vf3_ecr_item");
							}
						}
					}
				} else {
