...
public class AFSSORProcessTrigger_Handler extends AbstractHandler {
	private TCSession session = null;
	private DataManagementService dmService = null;
	private AFSSORProcessTrigger_Dialog dlg = null;
...
//	private final static String PURCHASE_GROUP = "3_IN_1_PDU_DCDC_OBC.Direct Pur.Purchase.VINFAST;BATTERY_PACK.Direct Pur.Purchase.VINFAST;BIW-DC.Direct Pur.Purchase.VINFAST;EE.Direct Pur.Purchase.VINFAST;EXT-AFS.Direct Pur.Purchase.VINFAST;INT.Direct Pur.Purchase.VINFAST;PWT-CHS.Direct Pur.Purchase.VINFAST;SEATS.Direct Pur.Purchase.VINFAST;SEATS_AND_RESTRAINTS.Direct Pur.Purchase.VINFAST;STANDARD.Direct Pur.Purchase.VINFAST";
	private final static String[] ALLOWED_OBJ_TYPES = { "VF4_DesignRevision", "VF3_Scooter_partRevision", "VF4_Service_KitRevision", "VF4_SSTRevision", "Vf8_AcarPartRevision", "VF7_service_chptRevision", "VF3_manuf_partRevision" };
	...
	public Object execute(ExecutionEvent arg0) throws ExecutionException {
		try {
			...
			String[] workprocessDataForm = new String[] { "AFS_SOR_Release_Process" };
			...
			String[] preValues = TCExtension.GetPreferenceValues("VF_AFS_SOR_PROCESS_SETTING", session);
			...
			dlg = new AFSSORProcessTrigger_Dialog(new Shell());
			dlg.create();
			...
			dlg.cbWorkflow.addListener(SWT.Modify, new Listener() {
				...
				public void handleEvent(Event arg0) {
					...
					dlg.lblAllowedObjectType.setText("Allowed Object Type: " + String.join(",", ALLOWED_OBJ_TYPES));
				}
			});
		...
		if (targetComp.length > 0) {
			for (InterfaceAIFComponent comp : targetComp) {
				if (comp instanceof TCComponentBOMLine) {
					...
					if (!Arrays.asList(ALLOWED_OBJ_TYPES).contains(type))
						continue;
