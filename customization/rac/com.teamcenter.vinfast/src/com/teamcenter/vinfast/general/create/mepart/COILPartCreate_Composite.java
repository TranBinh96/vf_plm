...
public class COILPartCreate_Composite extends MEPartCreateAbstract {
...
//	private Label lblNameWarning;
	...
	public COILPartCreate_Composite(Composite c) {
		super(c);
		...
		Listener coilGenNameListener = new Listener() {
			@Override
			public void handleEvent(Event arg0) {
				generateCOILName();
			}
		...
		cbMaterial.addListener(SWT.Modify, coilGenNameListener);
		...
		cbCoating.addListener(SWT.Modify, coilGenNameListener);
		...
		txtThickness.addListener(SWT.Modify, coilGenNameListener);
		...
		txtWidth.addListener(SWT.Modify, coilGenNameListener);
	...
	public String createNewItem(TCComponent selectedObject, boolean isOpenOnCreate) {
		...
		// for COIL Part
		String coilMaterial = cbMaterial.getText();
		String coilCoating = cbCoating.getText();
		double coilThickness = 0;
		try {
			coilThickness = Double.parseDouble(txtThickness.getText());
		} catch (Exception e) {
			e.printStackTrace();
		}
		double coilWidth = 0;
		try {
			coilWidth = Double.parseDouble(txtWidth.getText());
		} catch (Exception e) {
			e.printStackTrace();
		}
		boolean isCoilPurchase = ckbIsPurchase.getSelection();

		if (!checkPartExistSpec(coilMaterial, coilCoating, coilThickness, coilWidth, null)) {
	...
	private boolean coilValidate() {
		...
		if (name.contains("_")) {
			...
			if (str[0].compareToIgnoreCase("COIL") != 0) {
				isFormatValid = false;
//				lblNameWarning.setText("Prefix name only available for COIL.");
			} else {
				if (str.length == 5) {
					// material
		...
		if (isFormatValid)
			...
			lblNameWarning.setText("Please follow naming rule: Prefix_Material_Coating_Thickness_Width. Prefix: COIL.");
...
//			subCategory = "COIL";
		...
		txtName.setText("COIL" + "_" + material + (coating.isEmpty() ? "" : "_" + coating) + "_" + thickness + "_" + width);
	}
