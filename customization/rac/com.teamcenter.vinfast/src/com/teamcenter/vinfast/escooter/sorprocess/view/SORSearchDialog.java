...
public class SORSearchDialog extends AbstractSWTDialog {
	...
	public SORSearchDialog(Shell parent, int style, String _SearchType) {
		super(parent, style);
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblSORNumber.setText("SOR Number");
		...
		txtSORNumber.addListener(SWT.KeyUp, new Listener() {
			...
			public void handleEvent(Event evt) {
				if (evt.keyCode == SWT.CR || evt.keyCode == SWT.KEYPAD_CR) {
					getSORDoc(session, txtSORNumber, txtSORName, cbOwner);
				}
			}
		});
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText("SOR Search Dialog");
	}
	...
	private void getSORDoc(TCSession session, Text sorNumber, Text sorName, Combo cbOwner) {
		TCComponent[] objects = null;
		String id = sorNumber.getText();
		String name = sorName.getText();
		String owningUser = "";
		...
		if (id.equals("") && name.equals("") && owningUser.equals("")) {
			...
			queryInput.put("Type", searchType);
			objects = Query.queryItem(session, queryInput, searchType.contains("Revision") ? "Item Revision..." : "Item...");
