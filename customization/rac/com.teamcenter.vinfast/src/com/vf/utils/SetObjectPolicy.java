...
public class SetObjectPolicy {
	...
	public static void setOperationPolicy(TCSession session){ 

		SessionService sessionService = SessionService.getService(session);

		ObjectPropertyPolicy policy = new ObjectPropertyPolicy();
		
		policy.addType("BOMLine",new String[] {"VF3_torque_info","VL5_torque_inf","bl_quantity","Mfg0used_equipment","Mfg0all_material","al_activity_time_system_unit_time","me_cl_source","bl_me_activity_lines","al_activity_work_time","bl_parent","bl_rev_awp0Item_item_id","bl_item_object_name"});
		policy.addType("MEOPRevision",new String[] {"object_name","last_mod_user","last_mod_date","creation_date","item_id","item_revision_id","VF4_history_jes"});
		sessionService.setObjectPropertyPolicy(policy); 

	}
	
	public static void setChangePolicy(TCSession session){ 
