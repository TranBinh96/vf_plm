...
public class VESMaterialTransferValidate {
	public VESMaterialTransferValidate() {
	...
	public static TCComponentItemRevision getLatestValidReleasedRevision(TCSession session, TCComponentItem item, ArrayList<String> validStatus) throws TCException {
		boolean isMBOMRole = false;
		try {
			String currentRole = session.getRole().getRoleName();
			// isMBOMRole = currentRole.contains("MB Engineer") ||
			// currentRole.contains("DBA");
			isMBOMRole = currentRole.contains("DBA");
		} catch (TCException e) {
			e.printStackTrace();
		}
		...
		if (isMBOMRole && latestReleasedRev == null) {
			latestReleasedRev = item.getLatestItemRevision();
			itemRevs = item.getWorkingItemRevisions();
			if (itemRevs.length > 0) {
				latestReleasedRev = (TCComponentItemRevision) itemRevs[0];
			}
		}
