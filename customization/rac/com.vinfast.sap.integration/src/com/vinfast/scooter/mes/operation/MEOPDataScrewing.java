...
public class MEOPDataScrewing extends MEOPDataAbstract {
	@Override
	...
	void xmlAppendBody(StringBuffer dataString) {
		dataString.append("<Operation ID=\"" + this.getMEOPID() + "\" Revision=\"" + this.getMEOPRevID() + "\" OperationType=\"" + this.getMEOPType() + "\">");
		...
		ArrayList<String> toolData = this.getToolList();

		if (toolData.isEmpty() == false) {

			for (int i = 0; i < toolData.size(); i++) {
				dataString.append("<Tool ID=\"" + toolData.get(i).trim() + "\" ProgramNumberID=\"" + this.getProgramID().trim() + "\"/>");
			}
		}
	...
	public String isValidOperation() {
		if (getToolList().isEmpty()) {
			return "PLM: Screwing required atleast one Tool ID. Please add tool and fill program ID";
		} else if (getProgramID().isEmpty()) {
			return "PLM: Program ID is empty. Please fill program ID";
		}
		return "";
	}
}
