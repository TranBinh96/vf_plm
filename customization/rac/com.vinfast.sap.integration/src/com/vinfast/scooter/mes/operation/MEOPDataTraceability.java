...
public class MEOPDataTraceability extends MEOPDataAbstract {
	...
	void xmlAppendBody(StringBuffer dataString) {
		...
		dataString.append("<IsMHU>" + this.getIsMHU() + "</IsMHU>");
		dataString.append("<IsVCU>" + this.getIsVcu() + "</IsVCU>");
		dataString.append("</MaterialLists>");
		dataString.append("</MaterialToVerify>");
	...
	public String isValidOperation() {
		if (this.getMaterialList().isEmpty()) {
			return "PLM: \"Traceability\" operation required only one Traceable Part";
		}
		if (this.getMaterialList().size() >= 2 && this.getChildItems() != null) {
			String result = UIGetValuesUtility.hasOnlyColorParts(this.getChildItems(), "EScooter");
			if (!result.equals("")) {
				return result;
			}
		}
