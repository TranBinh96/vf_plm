...
@Component({
  selector: 'app-data-puller-sub-scription-list',
  templateUrl: './data-puller-sub-scription-list.component.html',
  styleUrls: ['./data-puller-sub-scription-list.component.css']
})
export class DataPullerSubScriptionListComponent implements OnInit {
  id: string;
  ...
  loadData() {
    ...
    this.dataPullerService.DataPullerSubscriptionHandler_GetAll(this.searchData).subscribe(x => {
  ...
  onChangeEvent(event){
    if(event.length > 0) this.searchData.EventID = event[0].value;
    else this.searchData.EventID = "";
    this.loadData();
  }
}
