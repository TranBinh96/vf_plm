...
import { SapService } from 'src/app/_services/sap.service';

@Component({
  selector: 'app-sap-masterial-master-list',
  templateUrl: './sap-masterial-master-list.component.html',
  styleUrls: ['./sap-masterial-master-list.component.css']
})
export class SapMasterialMasterListComponent implements OnInit {
  id: string;
  ...
  loadData() {
    ...
    this.sapService.SapMaterialMaster_GetAll(this.searchData).subscribe(x => {
  ...
  loadDataForm() {
    this.dataFormService.SapMaterialMasterList_GetDataForm().subscribe(x => {
      this.dataForm = x;
